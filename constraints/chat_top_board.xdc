...
# Pinout copied verbatim from example/project-template-xilinx/.../io.xdc
# for xc7a200tfbg484-2. Any deviation here puts the FPGA on pins that
# either don't connect to the SRAM at all or belong to QSPI Flash /
# PSRAM, which silently breaks the visual asset path.
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# BaseRAM external IO timing.
# clk_pix = 40 MHz (25 ns). Async SRAM (IS61WV25616 family, tAA ~10 ns).
# Address/control launched from clk_pix-domain logic; data captured by
# the s1_asset_sram_data register on the next rising edge. Budget:
#   FPGA Tco  + tAA  + board flight  + FPGA Tsu  <  25 ns
# Tune if the SRAM speed grade or trace lengths differ.
#
# The pixel clock is created by the Clocking Wizard from the clk_out1
# port; Vivado names that generated clock `clk_out1_ip_pll` (some
# versions just `clk_out1`). The wildcard matches both.
# ---------------------------------------------------------------------
set sram_pix_clk [get_clocks clk_out1*]
set sram_ctrl_ports [get_ports {base_ram_addr[*] base_ram_be_n[*] \
                                base_ram_ce_n base_ram_oe_n base_ram_we_n}]
set sram_data_ports [get_ports {base_ram_data[*]}]
set_output_delay -clock $sram_pix_clk -max  5.0 $sram_ctrl_ports
set_output_delay -clock $sram_pix_clk -min -2.0 $sram_ctrl_ports
set_input_delay  -clock $sram_pix_clk -max 15.0 $sram_data_ports
set_input_delay  -clock $sram_pix_clk -min  0.0 $sram_data_ports
