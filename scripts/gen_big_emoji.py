...
def emit_tokens_svh(emojis):
    # Encoder branches dropped verbatim into be_top.sv's encoded_line_payload
    # priority chain via `include. Each branch starts with `else if` (the
    # previous block's `end` is in be_top.sv) and ends with `end`.
    lines = [
        "// AUTO-GENERATED by scripts/gen_big_emoji.py -- do not edit by hand.",
    ...
    for i, (name, _, _) in enumerate(emojis):
        ...
        body[-1] = body[-1] + ") begin"
        body.append(f"    encoded_line_payload[dst*8 +: 8] = 8'h{anchor:02X};  // \\{name}")
        body.append(f"    src += {n_chars + 1};")
        body.append( "    dst += 1;")
        body.append( "end")
        lines.extend(body)
    lines.append("")
