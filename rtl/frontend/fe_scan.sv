...
//   screen 2..34   33 history rows (ring window, 64-deep ring)
//   screen 35      separator (always blank)
//   screen 36      input bar (text_ram row INPUT_ROW)
//
...
(
    ...
    input  msg_len_t               input_cursor,       // upper bits unused
    ...
    localparam int HIST_SCREEN_END   = HIST_SCREEN_START + N_HIST_VISIBLE - 1;  // = 34
    localparam int SEP_SCREEN_ROW    = HIST_SCREEN_END + 1;       // = 35
    localparam int INPUT_SCREEN_ROW  = HIST_SCREEN_END + 2;       // = 36
    ...
    always_comb begin
        ...
        else if (s0_screen_row == HWIDTH'(INPUT_SCREEN_ROW))
            s0_text_ram_row = FE_ROW_W'(INPUT_ROW);
        else
    ...
    // Cursor cell match: input row, col == INPUT_PREFIX_LEN + input_cursor.
    logic at_cursor_cell;
    assign at_cursor_cell =
        in_connected
        && (s1_text_ram_row == FE_ROW_W'(INPUT_ROW))
        && (s1_text_ram_col == (FE_COL_W'(INPUT_PREFIX_LEN) + FE_COL_W'(input_cursor)));
    ...
    always_comb begin
        ...
        if (s1_text_ram_row == FE_ROW_W'(TITLE_ROW)) begin
            ...
            bg_g = COL_TITLE_BG_G;
            bg_b = COL_TITLE_BG_B;
        end else if (s1_text_ram_row == FE_ROW_W'(INPUT_ROW)) begin
            bg_r = COL_INPUT_BG_R;
            bg_g = COL_INPUT_BG_G;
            bg_b = COL_INPUT_BG_B;
        end
