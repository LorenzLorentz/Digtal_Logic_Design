...
//   DATA frames:         apply alternating-bit duplicate filter.
//                        Match  -> deliver to backend, enqueue ACK,
//                                  toggle expected_rx_seq.
//                        Dup    -> enqueue ACK only.
//   Control frames:      HELLO / REHELLO / USERNAME / GOODBYE always
//   (HELLO etc.)         deliver to backend AND enqueue ACK; the
//                        expected_rx_seq counter is NOT consulted.
//                        Rationale: a peer reboot resets its tx_seq,
//                        and we don't want a stale expected_rx_seq to
...
(
    ...
    seq_t                        expected_rx_seq_q;   // only bit[0] used
    ...
    assign incoming_dup = frame_is_data
                       && (frame_in_seq[0] != expected_rx_seq_q[0]);
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Toggle expected_rx_seq only on accepted (non-dup) DATA delivery.
            if (state_q == S_QUEUE_ACK && ack_q_ready
              && !dup_q && (type_q == 3'(FRAME_DATA))) begin
                expected_rx_seq_q <= {7'b0, ~expected_rx_seq_q[0]};
            end
        end
    end
